...
function Start()
	...
	blockImage = {}
	blockImage.image = LoadImage("gme/block.png")
	blockImage.size = 32
...
function UpdateBullets()
	...
	for i = #bullets, 1, -1 do
		...
		elseif bullets[i].x < -bulletImage.size then
			table.remove(bullets, i)
		else for j = #blocks, 1, -1 do
			if BoxesOverlap(bullets[i].x, bullets[i].y, bulletImage.size, bulletImage.size, blocks[j].x, blocks[j].y, blockImage.size, blockImage.size) then
				table.remove(bullets, i)
...
function Draw()
	...
	for i = 1, #blocks do
		DrawImage(blockImage.image, 0, blocks[i].x, blocks[i].y, blockImage.size)
	end
